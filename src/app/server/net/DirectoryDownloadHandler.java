...
public class DirectoryDownloadHandler extends Handler{
    private final FileStorageService fileSystemService;
    private final SessionService sessionService;

    public DirectoryDownloadHandler(Transport transport, IO io, FileStorageService fileStorageService, SessionService sessionService) {
        super(transport, io);
    ...
    public void handle(Message message) {
        ...
        if (!fileSystemService.fileExists(username, filename)) {
            throw new ServerException("file '" + filename + "' not found");
        }
